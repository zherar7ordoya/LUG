...
namespace Negocio
{
    public class BLLUsuario
    {
        ...
        {  ///Declaro el objeto DataSet para guardar los datos y luego pasarlos a lista
            ...
            if (Ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow fila in Ds.Tables[0].Rows)
                {
                    ...
                    // *********************************************************
                    /* Cambié el código original (del profe) y no se produjo el
                     * problema que él anticipó (caída de la aplicación ante un
                     * valor null). Ha de ser en otro contexto. Si se deja el 
                     * código del profe, también se malogra el orden de columnas
                     * del DGV, no sé por qué). */

                    //if (fila["mail"] is DBNull) { oBEusu.Usuario = null; }
                    //else { oBEusu.Usuario = fila["mail"].ToString(); }

                    //if (fila["password"] is DBNull) { oBEusu.Psw = null; }
                    //else { oBEusu.Psw = fila["password"].ToString(); }
        ...
        // No está bien la lógica. Para que se cumpla, el DGV no debe haber sido
        // nunca seleccionado.
